# anomaly.py
# Функції генерації АВ та очищення (з лекції L_1_3)
...
def Model_NORM_AV(S0_real, nAV_pct=5, Q_AV=3, seed=42):
    ...
    SV_AV = S0_real.copy()
    # рівномірно розподілені номери АВ в межах вибірки (як у лекції)
    SAV   = [int(rng.integers(1, n)) for _ in range(nAV)]
    # аномальна випадкова похибка з нормальним законом (як у лекції)
...
def Sliding_Window_AV_Detect_sliding_wind(S0, n_Wind):
    """
    Виявлення та очищення АВ методом sliding window (з лекції L_1_3).
    ...
    # формування очищеної вибірки (з лекції)
    S0_Midi = np.zeros(iter)
    for j in range(iter):
        S0_Midi[j] = Midi[j]
    for j in range(n_Wind):       # перші n_Wind точок — без змін
        S0_Midi[j] = S0[j]
    return S0_Midi